...
export class PanelCapitanComponent implements OnInit {
  ...
  async ngOnInit(): Promise<void> {
    ...
    // Este panel solo puede usarlo un capitan. Si entra otro rol, se manda al login.
    if (!session || session.tipo !== 'capitan') {
  ...
  protected async createTeam(payload: { name: string; competition: string; captainUsername: string }): Promise<void> {
    ...
    try {
      // Cuando el capitan crea el equipo, guardamos tambien el nombre en la sesion
      // para no tener que recargar toda la informacion manualmente.
